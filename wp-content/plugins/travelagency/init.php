<?php
/*
Plugin Name: Travel Agency
Description: Plugin for Travel Agency
Version: 1
Author: naantam.in
Author URI: naantam.in
*/
// function to create the DB / Options / Defaults					
function sync_travelagency_install() {
...
register_activation_hook(__FILE__, 'sync_travelagency_install');

//menu items
add_action('admin_menu','sync_travelagency_modifymenu');
function sync_travelagency_modifymenu() {
	
	//this is the main item for the menu
	add_menu_page('Travelagency', //page title
	'Travelagency', //menu title
	'manage_options', //capabilities
	'sync_travelagency_list', //menu slug
	'sync_travelagency_list' //function
	);
	
	//this is a submenu
	add_submenu_page('sync_travelagency_list', //parent slug
	'Add New Travelagency', //page title
	'Add New Travelagency', //menu title
	'manage_options', //capability
	'sync_travelagency_create', //menu slug
	'sync_travelagency_create'); //function
	
	//this submenu is HIDDEN, however, we need to add it anyways
	add_submenu_page(null, //parent slug
	'Update Travelagency', //page title
	'Update', //menu title
	'manage_options', //capability
	'sync_travelagency_update', //menu slug
	'sync_travelagency_update'); //function
}
define('tatroomstravelagency', plugin_dir_path(__FILE__));
require_once(tatroomstravelagency . 'sync_travelagency-list.php');
require_once(tatroomstravelagency . 'sync_travelagency-create.php');
require_once(tatroomstravelagency . 'sync_travelagency-update.php');

//Frontend Travelagency Registration Form
function travelagency_registration_form(){
	ob_start();
...
<section id="contact">
	<div class="container">
	  <h2 class="h2 ac">Travel Agency Registration</h2>
	  <div class="row">
...
add_shortcode('cleopatra_travelagency_registration', 'travelagency_registration_form');

function travelagency_account_edit(){
    ob_start();
    if(!isset($_SESSION['user'])){
...
add_shortcode('cleopatra_travelagency_account_edit', 'travelagency_account_edit');

function travelagency_login_form(){
	ob_start();
	if(isset($_SESSION['user'])){
	...
	if(isset($_POST['login'])){
		...
		if(count($rows)==0){
			...
			if(count($rows_new)>0){
				...
				$_SESSION['user'] = array('user_id'=>$rows[0]->id,'type'=>'travelagency');
...
<section id="contact">
	<div class="container">
	  <h2 class="h2 ac">Travelagency Login</h2>
	  <div class="row">
	      <div class="col-lg-12 col-md-12 col-sm-12 col-12 col contact_form">
	          <?php echo $message1; ?>
	      </div>
		 ...
		 <div role="form">
			<form method="post" action="" enctype="multipart/form-data">
				...
				<div class="form-group">
					...
					<a href="<?php echo get_site_url(); ?>/travelagency-registration/">Register</a> | <a href="javascript:void(0);">Forget Passoword</a>
				</div>
			</form>
		</div> 
	   </div>
...
add_shortcode('cleopatra_travelagency_login', 'travelagency_login_form');

function travelagency_logout(){
	ob_start();
	if(isset($_SESSION['user'])){
		unset($_SESSION['user']);
...
add_shortcode('cleopatra_travelagency_logout', 'travelagency_logout');

function travelagency_change_password(){
    ob_start();
    if(!isset($_SESSION['user'])){
	    $_SESSION['message'] = 'You are not logged in.';
	    wp_redirect(get_site_url().'/message/');
...
add_shortcode('cleopatra_travelagency_change_password', 'travelagency_change_password');