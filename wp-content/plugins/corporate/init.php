<?php
/*
Plugin Name: Corporate
Description: Plugin for corporate
Version: 1
Author: naantam.in
Author URI: naantam.in
*/
// function to create the DB / Options / Defaults					
function sync_corporate_install() {
...
register_activation_hook(__FILE__, 'sync_corporate_install');

//menu items
add_action('admin_menu','sync_corporate_modifymenu');
function sync_corporate_modifymenu() {
	
	//this is the main item for the menu
	add_menu_page('Corporate', //page title
	'Corporate', //menu title
	'manage_options', //capabilities
	'sync_corporate_list', //menu slug
	'sync_corporate_list' //function
	);
	
	//this is a submenu
	add_submenu_page('sync_corporate_list', //parent slug
	'Add New Corporate', //page title
	'Add New Corporate', //menu title
	'manage_options', //capability
	'sync_corporate_create', //menu slug
	'sync_corporate_create'); //function
	
	//this submenu is HIDDEN, however, we need to add it anyways
	add_submenu_page(null, //parent slug
	'Update Corporate', //page title
	'Update', //menu title
	'manage_options', //capability
	'sync_corporate_update', //menu slug
	'sync_corporate_update'); //function
}
define('tatroomscorporate', plugin_dir_path(__FILE__));
require_once(tatroomscorporate . 'sync_corporate-list.php');
require_once(tatroomscorporate . 'sync_corporate-create.php');
require_once(tatroomscorporate . 'sync_corporate-update.php');

//Frontend Corporate Registration Form
function corporate_registration_form(){
	ob_start();
...
<section id="contact">
	<div class="container">
	  <h2 class="h2 ac">Corporate Registration</h2>
	  <div class="row">
...
add_shortcode('cleopatra_corporate_registration', 'corporate_registration_form');

function corporate_account_edit(){
    ob_start();
    if(!isset($_SESSION['user'])){
...
//update
    if (isset($_POST['update'])) {
        ...
        $sync_corporate = $wpdb->get_results($wpdb->prepare("SELECT * from $table_name where id=%s", $id));
        
        foreach ($sync_corporate as $s) {
...
add_shortcode('cleopatra_corporate_account_edit', 'corporate_account_edit');
	...
	if(isset($_POST['login'])){
		...
		if(count($rows)==0){
			...
			if(count($rows_new)>0){
				...
				$_SESSION['user'] = array('user_id'=>$rows[0]->id,'type'=>'corporate');
...
<section id="contact">
	<div class="container">
	  <h2 class="h2 ac">Corporate Login</h2>
	  <div class="row">
	      <div class="col-lg-12 col-md-12 col-sm-12 col-12 col contact_form">
	          <?php echo $message1; ?>
	      </div>
		 ...
		 <div role="form">
			<form method="post" action="" enctype="multipart/form-data">
				...
				<div class="form-group">
					...
					<a href="<?php echo get_site_url(); ?>/corporate-registration/">Register</a> | <a href="javascript:void(0);">Forget Passoword</a>
				</div>
			</form>
		</div> 
	   </div>
...
add_shortcode('cleopatra_corporate_login', 'corporate_login_form');
...
add_shortcode('cleopatra_corporate_logout', 'corporate_logout');

function corporate_change_password(){
    ob_start();
    if(!isset($_SESSION['user'])){
	    $_SESSION['message'] = 'You are not logged in.';
	    wp_redirect(get_site_url().'/message/');
...
add_shortcode('cleopatra_corporate_change_password', 'corporate_change_password');