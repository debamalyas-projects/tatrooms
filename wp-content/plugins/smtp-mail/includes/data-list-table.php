...
class SMTPMail_Data_List_Table extends WP_List_Table {
    ...
    function column_title($item){
        ...
        //Build row actions
        $actions = array(
            'detail' => '<a href="'. esc_url( admin_url('options-general.php?page='.$page.'&tab=list&action=detail&code='.$item['id'] ) ).'">Detail</a>',
            'delete' => '<a href="'. esc_url( admin_url('options-general.php?page='.$page.'&tab=list&action=delete&code='.$item['id'] ) ).'">Delete</a>',
        );
...
function smtpmail_render_customer_detail_form( $item )
{
    ...
    <form action="<?php echo esc_url( admin_url('options-general.php?page='.$page.'&tab=detail&code=' . $item->id ) );?>" method="post" class="smtpmail_detail_form">
        ...
        <div class="message-box"><?php echo nl2br( $message );?></div>
        <?php if( $whois_link!='' ):?>
        <h3>
